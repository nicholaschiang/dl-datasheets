...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest=Flags.FV_CODE,
        metavar="FV_CODE",
        default="ffe002af",  # op-amp
        help="The FV code of the part family on Digikey (default op-amps)",
    )
    parser.add_argument(
        "--encrypted",
        dest=Flags.KEEP_ENCRYPTED,
        action="store_true",
        default=False,
        help="Do not filter encrypted PDFs",
    )
    parser.add_argument(
        "--skip_csv",
        dest=Flags.SKIP_CSV_DL,
        action="store_true",
        default=False,
        help="Do not redownload the CSV.",
    )
    parser.add_argument(
        "--skip_pdf",
        dest=Flags.SKIP_PDF_DL,
        action="store_true",
        default=False,
        help="Do not redownload the PDFs.",
    )
    parser.add_argument(
        "--ocr",
        dest=Flags.KEEP_OCR,
        action="store_true",
        default=False,
        help="Do not filter PDFs that need OCR",
    )
    parser.add_argument(
        "--duplicates",
        dest=Flags.KEEP_DUPLICATES,
        action="store_true",
        default=False,
        help="Do not filter duplicate PDFs (NOT IMPLEMENTED)",
    )
...
def check_str_is_dir(input_path):
    ...
    if not os.path.exists(directory):
        raise argparse.ArgumentTypeError(
            "This directory does not exist. Please create it first."
        )
